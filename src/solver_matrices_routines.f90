            ...
            IF(ASSOCIATED(ROW_DOMAIN_MAP)) THEN
              DO matrix_idx=1,SOLVER_MATRICES%NUMBER_OF_MATRICES
                ...
                IF(ASSOCIATED(SOLVER_MATRIX)) THEN
                  ...
                  IF(ASSOCIATED(COLUMN_DOMAIN_MAP)) THEN
                    ...
                    ENDIF
                    IF(ASSOCIATED(SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION) &
                      & .AND. SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION% &
                      & DECOMPOSITION_TYPE==DECOMPOSITION_EVERY_TYPE) THEN
                      CALL DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH(SOLVER_MATRIX%MATRIX,ERR,ERROR,*999)
                    ELSE
                    ...
                    IF(ASSOCIATED(SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION) &
                      & .AND. SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION% &
                      & DECOMPOSITION_TYPE==DECOMPOSITION_EVERY_TYPE) THEN
                      CALL DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH(SOLVER_MATRIX%SOLVER_VECTOR,ERR,ERROR,*999)
                    ELSE
              ...
              IF(SOLVER_EQUATIONS%LINEARITY==PROBLEM_SOLVER_NONLINEAR) THEN
                ...
                IF(ASSOCIATED(SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION) &
                  & .AND. SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION% &
                  & DECOMPOSITION_TYPE==DECOMPOSITION_EVERY_TYPE) THEN
                  CALL DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH(SOLVER_MATRICES%RESIDUAL,ERR,ERROR,*999)
                ELSE
...
!!TODO: what to do if there is no RHS
              ...
              IF(ASSOCIATED(SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION) &
                & .AND. SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION% &
                & DECOMPOSITION_TYPE==DECOMPOSITION_EVERY_TYPE) THEN
                CALL DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH(SOLVER_MATRICES%RHS_VECTOR,ERR,ERROR,*999)
              ELSE
