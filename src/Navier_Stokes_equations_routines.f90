            ...
            CASE(PROBLEM_SETUP_CONTROL_TYPE)
              SELECT CASE(PROBLEM_SETUP%ACTION_TYPE)
                CASE(PROBLEM_SETUP_START_ACTION)
                  ...
                  CALL CONTROL_LOOP_LABEL_SET(iterativeWhileLoop2,"1D Iterative Loop",ERR,ERROR,*999)
                  NULLIFY(simpleLoop)
    ...
    CASE(EQUATIONS_SET_STATIC_RBS_NAVIER_STOKES_SUBTYPE, &
      ...
      IF(decomposition%CALCULATE_FACES .AND. integratedBoundary) THEN
        ...
        DO componentIdx=1,3
          ...
          ! CALL Field_ParameterSetGetLocalElement(equationsSetField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
          !  & elementNumber,componentIdx+11,boundaryInPlaneVector1(componentIdx),err,error,*999)
          ! CALL Field_ParameterSetGetLocalElement(equationsSetField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
          !  & elementNumber,componentIdx+14,boundaryInPlaneVector2(componentIdx),err,error,*999)
        END DO
        ...
        DO faceIdx=1,dependentBasis%NUMBER_OF_LOCAL_FACES
          ...
          DO gaussIdx=1,faceQuadratureScheme%NUMBER_OF_GAUSS
            ...
            DO componentIdx=1,dependentVariable%NUMBER_OF_COMPONENTS-1
              ...
              END IF
              ! !DEBUG
              ! IF(componentIdx/=normalComponentIdx) THEN
              !   DO j=1,3
              !     correct1D(i,j)=DOT_PRODUCT(pointMetrics%GU(componentIdx,:),pointMetrics%DX_DXI(j,:))
              !   END DO
              !   i = i+1
              ! END IF
            END DO
...
999 CALL ERRORS("NavierStokes_FiniteElementFaceIntegrate",err,error)
  ...
  SUBROUTINE NavierStokes_CalculateBoundaryFlux(equationsSet,coupledEquationsSet,err,error,*)
    ...
    REAL(DP) :: pressureGauss,faceTraction,mu,muScale,normal,rho
    ...
    REAL(DP) :: couplingFlow,couplingStress,p1D,q1D,a1D,p3D
...
999 CALL ERRORS("NavierStokes_Couple1D0D",err,error)
  ...
  SUBROUTINE NavierStokes_Couple3D1D(controlLoop,err,error,*)
    ...
    CASE(PROBLEM_MULTISCALE_NAVIER_STOKES_SUBTYPE)
      ...
      relativeCouplingTolerance = iterativeLoop%ABSOLUTE_TOLERANCE
    ...
    DO nodeIdx=1,numberOfLocalNodes1D
      ...
      IF(ABS(normalWave(1))>ZERO_TOLERANCE .OR. ABS(normalWave(2))>ZERO_TOLERANCE) THEN
        ...
        IF(ABS(flowError) < flowTolerance .AND. ABS(stressError) < stressTolerance) THEN
          boundaryConverged(boundaryNumber) = .TRUE.
        ELSE IF( ABS(flowError) < absoluteCouplingTolerance .AND. ABS(stressError) < absoluteCouplingTolerance) THEN
          boundaryConverged(boundaryNumber) = .TRUE.
        END IF
...
999 CALL ERRORS("NavierStokes_FiniteElementPreResidualEvaluate",err,error)
  ...
  SUBROUTINE NavierStokes_ControlLoopPostLoop(controlLoop,err,error,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: error !<The error string
    !Local Variables
    TYPE(CONTROL_LOOP_TYPE), POINTER :: subloop,subloop2
    TYPE(SOLVER_TYPE), POINTER :: navierStokesSolver,solver
    ...
    INTEGER(INTG) :: subloopIdx,subloopIdx2,iteration
    character(70) :: label
