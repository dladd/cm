...
For more extensive discussion of this procedure, along with other weak forms of the PDEs, we refer to \cite{gresho_isothermal:2000}. 

% Boundary conditions
From this weak form, we see natural (Neumann) boundary conditions arising as a direct result of the integration. Neumann boundary conditions will therefore consist of a pressure term and viscous stress acting normal to a given boundary. 
\begin{gather}
...
\end{gather}
It should also be noted that in defining Neumann conditions, we are in effect specifying a force at the given boundary, and this will involve direct modification of the right hand side of our system. We will address this process in further detail in our discussion about our implementation of the coupled multidomain method \cite{vignon:2006}.

Dirichlet boundary conditions for velocity may also be defined on a boundary $\Gamma_D$:
\begin{gather}
 \label{eqn:NSEDirichletBC} 
  \fnof{\vect{u}}{\vect{x},t} = \fnof{\vect{d}}{\vect{x},t} \quad \vect{x}\in\Gamma_{D}
\end{gather}
Where $\vect{d}$  is a vector of velocity values to be specified at the given velocity DOFs. This makes defining inlet and outlet velocities for system boundaries relatively simple, as values are simply applied as fixed. Wall boundaries may also be easily defined by setting their DOF velocity values to zero: 
\begin{gather}
 \label{eqn:NSEDirichletBC} 
  \fnof{\vect{u}}{\vect{x},t} = \vect{0} \quad \vect{x}\in\Gamma_{D}
\end{gather}
This will ensure that fluid velocity approaching the wall is parallel to it and will not penetrate. A third type of boundary condition may be implicitly applied to DOFs by leaving their values undefined in boundary regions. These 'do nothing' boundary conditions are useful in situations involving one unknown boundary or those involving symmetric geometries. However, 'do nothing' boundaries also imply a zero pressure boundary condition and are therefore dangerous in nonsymmetric branching conduits (like blood vessels) as the fluid will be driven by these pressure gradients, modifing flow field features \cite{galdi:2000}.

Creating well-posed boundary conditions with specified pressures is considerably more difficult than the velocity case. In effect, attempting to specify the scalar pressure values via Dirichlet conditions is the same as attempting to specify Neumann conditions with undefined $\grad{\fnof{\vect{u}}{\vect{x},t}}$ values, which creates two unknowns at the boundary (the other being the force term $\fnof{\vect{q}}{\vect{x},t}$). 
