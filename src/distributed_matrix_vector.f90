...
#include "petscversion.h"
 ...
 INTERFACE DistributedMatrix_DataRestore
  ...
  PUBLIC DISTRIBUTED_MATRIX_CREATE_FINISH,DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH,DISTRIBUTED_MATRIX_CREATE_START
  ...
  PUBLIC DISTRIBUTED_VECTOR_CREATE_FINISH,DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH,DISTRIBUTED_VECTOR_CREATE_START
...
998 CALL ERRORS("DISTRIBUTED_MATRIX_CREATE_FINISH",ERR,ERROR)
  ...
  !>Finishes the creation of a PETSc SEQAIJ matrix.
  SUBROUTINE DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH(DISTRIBUTED_MATRIX,ERR,ERROR,*)
    ...
    CALL ENTERS("DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH",ERR,ERROR,*998)
    ...
    IF(ASSOCIATED(DISTRIBUTED_MATRIX)) THEN
      ...
      ELSE
        SELECT CASE(DISTRIBUTED_MATRIX%LIBRARY_TYPE)
        CASE(DISTRIBUTED_MATRIX_VECTOR_CMISS_TYPE)
          CALL FLAG_ERROR("Sequential CMISS distributed matrix types not yet implemented.",ERR,ERROR,*998)
        CASE(DISTRIBUTED_MATRIX_VECTOR_PETSC_TYPE)
          CALL DISTRIBUTED_MATRIX_PETSC_SEQAIJ_CREATE_FINISH(DISTRIBUTED_MATRIX%PETSC,ERR,ERROR,*999)
        CASE DEFAULT
          LOCAL_ERROR="The distributed matrix library type of "// &
            & TRIM(NUMBER_TO_VSTRING(DISTRIBUTED_MATRIX%LIBRARY_TYPE,"*",ERR,ERROR))//" is invalid."
          CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
    ...
    CALL EXITS("DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH")
    RETURN
999 CALL DISTRIBUTED_MATRIX_FINALISE(DISTRIBUTED_MATRIX,DUMMY_ERR,DUMMY_ERROR,*998)
998 CALL ERRORS("DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH",ERR,ERROR)
    CALL EXITS("DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH")
    RETURN 1
  END SUBROUTINE DISTRIBUTED_MATRIX_SEQAIJ_CREATE_FINISH
...
998 CALL ERRORS("DISTRIBUTED_MATRIX_PETSC_CREATE_FINISH",ERR,ERROR)
  ...
  !>Finishes the creation of a CMISS distributed matrix.
  SUBROUTINE DISTRIBUTED_MATRIX_PETSC_SEQAIJ_CREATE_FINISH(PETSC_MATRIX,ERR,ERROR,*)
    ...
    CALL ENTERS("DISTRIBUTED_MATRIX_PETSC_SEQAIJ_CREATE_FINISH",ERR,ERROR,*999)
        ...
        IF(ASSOCIATED(ROW_DOMAIN_MAPPING)) THEN
          IF(ASSOCIATED(COLUMN_DOMAIN_MAPPING)) THEN
            ...
            CASE(DISTRIBUTED_MATRIX_BLOCK_STORAGE_TYPE)
              ...
              IF(ERR/=0) CALL FLAG_ERROR("Could not allocate PETSc matrix data.",ERR,ERROR,*999)
#if ( PETSC_VERSION_MAJOR >= 3 && PETSC_VERSION_MINOR >= 3 )
              CALL PETSC_MATCREATESEQDENSE(PETSC_COMM_SELF,PETSC_MATRIX%M,PETSC_MATRIX%N, &
                & PETSC_MATRIX%DATA_DP,PETSC_MATRIX%MATRIX,ERR,ERROR,*999)
#else
              CALL FLAG_ERROR("SEQAIJ matrix creation not yet implemented for PETSc version < 3.3.",ERR,ERROR,*999)
#endif              
            CASE(DISTRIBUTED_MATRIX_DIAGONAL_STORAGE_TYPE)
              PETSC_MATRIX%NUMBER_NON_ZEROS=PETSC_MATRIX%M
              PETSC_MATRIX%MAXIMUM_COLUMN_INDICES_PER_ROW=1
              ...
              PETSC_MATRIX%DIAGONAL_NUMBER_NON_ZEROS=1
              PETSC_MATRIX%OFFDIAGONAL_NUMBER_NON_ZEROS=0
              !Create the PETsc SEQAIJ matrix
#if ( PETSC_VERSION_MAJOR >= 3 && PETSC_VERSION_MINOR >= 3 )
              CALL PETSC_MATCREATESEQAIJ(PETSC_COMM_SELF,PETSC_MATRIX%M,PETSC_MATRIX%N, &
                & PETSC_NULL_INTEGER,PETSC_MATRIX%DIAGONAL_NUMBER_NON_ZEROS,PETSC_MATRIX%MATRIX,ERR,ERROR,*999)
#else              
              CALL FLAG_ERROR("SEQAIJ matrix creation not yet implemented for PETSc version < 3.3.",ERR,ERROR,*999)
#endif              
            CASE(DISTRIBUTED_MATRIX_COLUMN_MAJOR_STORAGE_TYPE)
    ...
    CALL EXITS("DISTRIBUTED_MATRIX_PETSC_SEQAIJ_CREATE_FINISH")
    RETURN
999 CALL DISTRIBUTED_MATRIX_PETSC_FINALISE(PETSC_MATRIX,DUMMY_ERR,DUMMY_ERROR,*998)
998 CALL ERRORS("DISTRIBUTED_MATRIX_PETSC_SEQAIJ_CREATE_FINISH",ERR,ERROR)
    CALL EXITS("DISTRIBUTED_MATRIX_PETSC_SEQAIJ_CREATE_FINISH")
    RETURN 1
  END SUBROUTINE DISTRIBUTED_MATRIX_PETSC_SEQAIJ_CREATE_FINISH
...
998 CALL ERRORS("DISTRIBUTED_VECTOR_CREATE_FINISH",ERR,ERROR)
  ...
  !>Finishes the creation a distributed vector
  SUBROUTINE DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH(DISTRIBUTED_VECTOR,ERR,ERROR,*)
    ...
    CALL ENTERS("DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(DISTRIBUTED_VECTOR)) THEN
      ...
      ELSE
        ...
        CASE(DISTRIBUTED_MATRIX_VECTOR_CMISS_TYPE)
          IF(ASSOCIATED(DISTRIBUTED_VECTOR%CMISS)) THEN
            CALL FLAG_ERROR("Sequential CMISS distributed vector types not yet implemented.",ERR,ERROR,*998)
          ELSE
            CALL FLAG_ERROR("Distributed vector CMISS is not associated.",ERR,ERROR,*999)
          ENDIF
        CASE(DISTRIBUTED_MATRIX_VECTOR_PETSC_TYPE)
          IF(ASSOCIATED(DISTRIBUTED_VECTOR%PETSC)) THEN
            CALL DISTRIBUTED_VECTOR_PETSC_SEQ_CREATE_FINISH(DISTRIBUTED_VECTOR%PETSC,ERR,ERROR,*999)
          ELSE
            CALL FLAG_ERROR("Distributed vector PETSc is not associated.",ERR,ERROR,*999)
          ENDIF
        CASE DEFAULT
    ...
    CALL EXITS("DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH")
    RETURN
999 IF(ASSOCIATED(DISTRIBUTED_VECTOR)) CALL DISTRIBUTED_VECTOR_FINALISE(DISTRIBUTED_VECTOR,DUMMY_ERR,DUMMY_ERROR,*998)
    DEALLOCATE(DISTRIBUTED_VECTOR)
998 CALL ERRORS("DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH",ERR,ERROR)
    CALL EXITS("DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH")
    RETURN 1
  END SUBROUTINE DISTRIBUTED_VECTOR_SEQ_CREATE_FINISH
...
998 CALL ERRORS("DISTRIBUTED_VECTOR_PETSC_CREATE_FINISH",ERR,ERROR)
  ...
  !>Finishes the creation of a sequential PETSc distributed vector.
  SUBROUTINE DISTRIBUTED_VECTOR_PETSC_SEQ_CREATE_FINISH(PETSC_VECTOR,ERR,ERROR,*)
    ...
    CALL ENTERS("DISTRIBUTED_VECTOR_PETSC_SEQ_CREATE_FINISH",ERR,ERROR,*998)
    ...
    IF(ASSOCIATED(PETSC_VECTOR)) THEN
      ...
      IF(ASSOCIATED(DISTRIBUTED_VECTOR)) THEN
        ...
        IF(ASSOCIATED(DOMAIN_MAPPING)) THEN
          !Create the PETSc vector
          PETSC_VECTOR%DATA_SIZE=PETSC_VECTOR%N
          CALL PETSC_VECCREATESEQ(PETSC_COMM_SELF,PETSC_VECTOR%N,PETSC_VECTOR%VECTOR,ERR,ERROR,*999)
          !Set up the Local to Global Mappings
          DO i=1,PETSC_VECTOR%N
            PETSC_VECTOR%GLOBAL_NUMBERS(i)=DOMAIN_MAPPING%LOCAL_TO_GLOBAL_MAP(i)-1
          ENDDO !i
    ...
    CALL EXITS("DISTRIBUTED_VECTOR_PETSC_SEQ_CREATE_FINISH")
    RETURN
999 IF(ALLOCATED(GLOBAL_NUMBERS)) DEALLOCATE(GLOBAL_NUMBERS)
    CALL DISTRIBUTED_VECTOR_PETSC_FINALISE(PETSC_VECTOR,DUMMY_ERR,DUMMY_ERROR,*998)
998 CALL ERRORS("DISTRIBUTED_VECTOR_PETSC_SEQ_CREATE_FINISH",ERR,ERROR)
    CALL EXITS("DISTRIBUTED_VECTOR_PETSC_SEQ_CREATE_FINISH")
    RETURN 1
  END SUBROUTINE DISTRIBUTED_VECTOR_PETSC_SEQ_CREATE_FINISH
